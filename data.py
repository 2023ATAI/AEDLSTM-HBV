...
from HBV import HBVfordata,load_para_for_pred
...
# ------------------------------------------------------------------------------------------------------------------------------
	#Partition and create datasets
        ...
        # Create a mid-forcing dataset；The default training data is 1990 to 2019.
        forcing_slice = forcing[..., [0, 1]]
        land_surface_slice = land_surface[..., -1:]
        result = np.concatenate((forcing_slice, land_surface_slice), axis=-1)
        para = load_para_for_pred(cfg)
        mid_forcing_list = cfg['mid_forcing_list']
        mid = HBVfordata(result, para, mid_forcing_list)
        max_values = np.nanmax(mid, axis=(0, 1, 2))
        min_values = np.nanmin(mid, axis=(0, 1, 2))
        for feature_idx, (max_val, min_val) in enumerate(zip(max_values, min_values)):
            print(f"Feature {feature_idx + 1}:")
            print(f"  Maximum Value: {max_val}")
            print(f"  Minimum Value: {min_val}")
        self.time_length_m, self.nlat_m, self.nlon_m, self.num_features_m = mid.shape

        max_values = np.nanmax(mid, axis=(0, 1, 2))
        max_indices = np.argwhere(mid == max_values[:, np.newaxis, np.newaxis, np.newaxis])

        # 输出结果
        print("max：", max_values)
        print("xingxi：")
        for idx, value in zip(max_indices, max_values):
            print(f"feat {idx[0]} max {value}，locat {tuple(idx[1:])}")

        if not os.path.exists(PATH + 'mid_forcing.npy'):
            mid_forcing = np.memmap(PATH + 'mid_forcing.npy', dtype=cfg['data_type'], mode='w+', shape=mid.shape)
            mid_forcing[:, :, :, :] = mid
            mid_forcing.flush()
            del mid_forcing
        mid_forcing = np.memmap(PATH + 'mid_forcing.npy', dtype=cfg['data_type'], mode='c',
                                shape=(self.time_length_m, self.nlat_m, self.nlon_m, self.num_features_m))
        print('[ATAI {d_p} work ] finish  generating mid_forcing, mid_forcing is {xts}'.format(d_p=cfg['workname'],
                                                                                               xts=mid_forcing.shape))
        # ------------------------------------------------------------------------------------------------------------------------------
        ...
        if not os.path.exists(PATH + 'x_train.npy'):
            ...
            self.time_length_f - N, self.nlat_f, self.nlon_f,
            self.num_features_f + self.num_features_l + self.num_features_m))
            x_train[:, :, :, :self.num_features_f] = forcing[:self.time_length_f - N]
            x_train[:, :, :, self.num_features_f:self.num_features_f + self.num_features_l] = land_surface[
                                                                                              :self.time_length_f - N]
            x_train[:, :, :, self.num_features_f + self.num_features_l:] = mid_forcing[:self.time_length_f - N]
            x_train.flush()
            del x_train
        x_train = np.memmap(PATH + 'x_train.npy', dtype=cfg['data_type'], mode='c', shape=(
        self.time_length_f - N, self.nlat_f, self.nlon_f,
        self.num_features_f + self.num_features_l + self.num_features_m))
        print('[ATAI {d_p} work ] finish  generating x_train data, x_train shape is {xts}'.format(d_p=cfg['workname'],
                                                                                                  xts=x_train.shape))
        # ------------------------------------------------------------------------------------------------------------------------------
        y_train = label[:self.time_length_f - N]
        np.save(PATH + 'y_train.npy', y_train)
        print('[ATAI {d_p} work ] finish  generating y_train data, y_train shape is {yts}'.format(d_p=cfg['workname'],
                                                                                                  yts=y_train.shape))
        # Create a test dataset ；The default for the test dataset is only 2020
        if not os.path.exists(PATH + 'x_test.npy'):
            x_test = np.memmap(PATH + 'x_test.npy', dtype=cfg['data_type'], mode='w+', shape=(
            N, self.nlat_f, self.nlon_f, self.num_features_f + self.num_features_l + self.num_features_m))
            x_test[:, :, :, :self.num_features_f] = forcing[self.time_length_f - N:]
            x_test[:, :, :, self.num_features_f:self.num_features_f + self.num_features_l] = land_surface[
                                                                                             self.time_length_f - N:]
            x_test[:, :, :, self.num_features_f + self.num_features_l:] = mid_forcing[self.time_length_f - N:]
            x_test.flush()
            del x_test
        x_test = np.memmap(PATH + 'x_test.npy', dtype=cfg['data_type'], mode='c', shape=(
        N, self.nlat_f, self.nlon_f, self.num_features_f + self.num_features_l + self.num_features_m))
        print('[ATAI {d_p} work ] finish  generating x_test data, x_test shape is {xts}'.format(d_p=cfg['workname'],
                                                                                                xts=x_test.shape))
        y_test = label[self.time_length_f - N:]
        print('[ATAI {d_p} work ] finish  generating y_test data, y_test shape is {yts}'.format(d_p=cfg['workname'],
                                                                                                yts=y_train.shape))
        del forcing, label, land_surface, mid_forcing
# ------------------------------------------------------------------------------------------------------------------------------
# normalize
        if self.normalize:
