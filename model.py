...
class LSTMDecoder(nn.Module):
    ...
    def forward(self, x):
        # Forward pass through LSTM
        out, _= self.lstm(x)
        #print("After Decoder shape:", out.shape)
        # Pass through fully connected layer
        out = self.drop(out)
        #print("After Dense layer shape:", out.shape)
        out = self.fc(out[:,-1,:])
        #print("After fc layer shape:", out.shape)
        return out
...
class LSTMAttentionEncoderDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm(x)
        #print("After LSTM shape:", out.shape)
        # Permute the output for attention layer
        out = out.permute(1, 0, 2)
        # Apply multi-head attention
        out, _ = self.attention(out, out, out)
        #print("After Attention shape:", out.shape)
        out = self.dense1(out)
        out = out.permute(1, 0, 2)
